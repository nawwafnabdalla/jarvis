...
| **C4b** | **Vault access scope** | **Vault reads are restricted to a fixed enumerated set of coarse annual aggregates: annual median and IQR of session ranges, realised volatility and spread, plus admissible day counts. One row per year per session. No conditioning on any other variable, no cross-tabs, no sub-daily breakdown.** No Stage 2 report of any kind reads vault years (2023–present) — that boundary is absolute. Within the non-vault range, Stage 2's initial report suite (R1, R2, R5) is further restricted to **2007–2014 only**, per AR-1 (Part 3, risk 7) — not the full 2007–2022 development+validation range this entry originally stated, which conflicted with AR-1 and is corrected here (D-067). Stage 0 runs on 2007–2022 only. | **amended — D-067, reconciling with AR-1** |
