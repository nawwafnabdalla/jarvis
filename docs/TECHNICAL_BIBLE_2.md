...
`session_terminal` marks features that are only defined once a session has **closed** (for example `pre_london_range`). These are the highest-risk features in the library: a naive implementation makes the completed Asian range available at 03:00 London time, which is a time machine. The framework enforces it: a `session_terminal` feature's value reveals the most recently **completed** instance of its own session — null for every bar before any instance has ever closed, and thereafter the last instance whose own window has closed, updating the moment a newer instance closes in turn (D-073). **Corrected by D-073, WP-020** — this previously read "constant thereafter for the remainder of the trading day," which assumed every session's own close falls safely before that trading day's own end. It does not for `new_york`, whose close coincides exactly with this project's trading-day rollover instant (both anchored to 17:00 `America/New_York`): under the old wording, `new_york`'s own session_terminal features could never reveal a value within their own trading day at all — not a coding bug, a gap in this definition itself, found only once a `new_york` session_terminal feature was actually built (R1/WP-020) and exercised the boundary the earlier five-day-old text never had to.
